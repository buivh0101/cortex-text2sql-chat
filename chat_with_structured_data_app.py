...
def answer_question_using_analyst(query: str):
    ...
    # Append analyst response to conversation
    ...
    # Display interpretation and suggestions (only if SQL is present)
    if has_sql:
    ...
    # If no SQL, yield the analyst texts as final answer and finish
    if not has_sql:
        final_text = "\n".join(parsed.get("texts", []))
        yield final_text
        return

    # Process SQL - display SQL and execute it
    ...
    # Generate final answer from SQL results using LLM streaming
    try:
